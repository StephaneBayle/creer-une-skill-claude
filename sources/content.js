...
const QUIZ = [
  {
    ...
    ok: 1,
    why: "Claude lit en permanence la description de chaque skill activée, et elle seule, pour décider s'il ouvre la skill. Une description floue fait une skill qui ne se déclenche jamais, ou qui se déclenche à tort. C'est la phrase la plus importante de la skill.",
  },
  {
...
const SLIDES = [
  ...
  {
    kind: "cards", sec: "s1", time: 75,
    title: "Un classeur de fiches méthode",
    caption: "Claude lit les titres… et n'ouvre que la bonne fiche.",
    cards: ["Compte rendu", "Devis", "Post LinkedIn", "Charte graphique", "Mail client"],
    pick: 0,
    notes: {
      objectif: "Donner une image mentale simple et durable de la skill.",
      script: "Imaginez un classeur de fiches méthode posé à côté de Claude. Sur chaque fiche : un titre, une phrase qui dit quand s'en servir, puis la marche à suivre. Au début de chaque conversation, Claude ne lit que les titres et ces petites phrases. Ça ne lui coûte presque rien, donc vous pouvez avoir beaucoup de fiches. Quand vous demandez « fais-moi le compte rendu de cette réunion », il reconnaît la fiche qui correspond, la sort du classeur et suit la méthode. Les autres fiches restent fermées. C'est ce qui rend les skills efficaces : elles ne surchargent pas Claude, elles arrivent au bon moment. Et vous n'avez pas à y penser : c'est Claude qui va chercher la fiche. Vous pouvez aussi l'appeler vous-même en tapant « / » dans la zone de message.",
  ...
  },
  {
    ...
    notes: {
      ...
      script: "Trois portes, une même skill. Chat, c'est la conversation classique, sur le web ou dans l'application : vous dites « aide-moi à créer une skill », Claude vous interroge, rédige, et propose de l'enregistrer. Cowork, c'est le mode où Claude travaille dans vos dossiers et produit de vrais fichiers. Petite actualité : Anthropic est en train de fusionner Chat et Cowork en un seul Claude. Selon votre offre, vous verrez encore deux onglets ou un seul (le déploiement a commencé pour Pro et Max, Team et Free suivront, et les administrateurs Enterprise sont prévenus à l'avance). Rien ne se perd : vos skills, projets et connecteurs suivent. Code, enfin, c'est Claude Code. Son nom fait peur, mais c'est simplement Claude qui travaille dans un dossier de votre ordinateur, avec plus de contrôle. Il est disponible dans un onglet de l'application de bureau, sans écran noir ni ligne de commande. Une bonne nouvelle pour finir : une skill enregistrée dans votre compte Claude vous suit dans les trois portes.",
  ...
  },
  {
    ...
    cols: [
      ["FaComments", "Chat", ["Rien à installer", "Guidé pas à pas", "Enregistré en 1 clic"], ["Ne voit pas vos dossiers", "Fichiers joints un à un"]],
  ...
  },
  {
    ...
    myths: [
      ...
      ["Je vais tout casser", "Il demande avant d'agir"],
  ...
  },
  {
    ...
    notes: {
      ...
      script: "Un : repérer une tâche qui revient au moins une fois par semaine et qui se fait toujours de la même façon. Deux : la décrire à Claude comme à un nouveau collègue, avec si possible un exemple réussi (un ancien compte rendu, un mail type). Trois : laisser Claude générer la skill ; il va vous poser des questions, répondez-y franchement. Quatre : tester, dans une nouvelle conversation, avec une demande naturelle, sans nommer la skill. Est-ce qu'elle se déclenche ? Est-ce que le résultat est bon ? Cinq : ajuster. Si elle ne se déclenche pas, c'est presque toujours la description qu'il faut revoir. Si le résultat n'est pas bon, ce sont les instructions ou l'exemple. On boucle entre quatre et cinq deux ou trois fois : c'est normal.",
      question: "",
  ...
  },
  {
    ...
    notes: {
      ...
      script: "Si la connexion le permet, faites la démo en direct dans Chat ; sinon, commentez la maquette. Tapez : « Aide-moi à créer une skill qui transforme mes notes de réunion en compte rendu à notre format. » Claude s'appuie sur sa skill intégrée de création de skills (« skill-creator ») et pose des questions : qui lit ces comptes rendus ? quelle longueur ? avez-vous un exemple ? Répondez, collez un ancien compte rendu réussi. Claude rédige le SKILL.md et vous le montre : faites lire la description à voix haute à la salle. Puis il propose d'enregistrer la skill : un clic, et elle apparaît dans Personnaliser › Skills. Enfin, testez : ouvrez une nouvelle conversation, collez des notes en vrac et dites simplement « tu peux m'en faire un compte rendu ? ». La skill doit se déclencher seule. Si la démo prend du retard, arrêtez-vous après l'enregistrement : le test sera fait pendant l'atelier.",
  ...
  },
  {
    ...
    notes: {
      ...
      script: "Premier secret : la description doit dire QUAND utiliser la skill, pas seulement ce qu'elle fait. « Compte rendu » ne suffit pas. Écrivez plutôt « Rédige un compte rendu de réunion à notre format. Utiliser quand l'utilisateur colle des notes de réunion ou demande un CR, une synthèse de réunion ou un relevé de décisions. » Mettez les mots que vous emploieriez vraiment. Deuxième secret : un exemple réussi vaut dix consignes. Claude imite très bien : donnez-lui un vrai compte rendu que vous aimez. Troisième secret : une skill, une tâche. Mieux vaut trois petites skills claires qu'une grosse skill « tout-en-un » que Claude ne saura pas quand déclencher. Et restez court : si vos instructions dépassent deux pages, c'est souvent qu'il y a deux skills.",
      question: "",
  ...
  },
  {
    ...
    rings: [
      ["FaUser", "Moi", "Télécharger le .zip et l'envoyer"],
